...
# ----------------------------------------------------------------------------
# Sample realized paths (demand deviation + hydrology regime) on the CONTINUOUS
...
# Replay the trained SDDP policy on a prescribed path (regime sequence, demand
...
# hydrology representation. It respects a KNOWN initial condition (e.g. a dry
# start, hInit=[1,0,0]) instead of hard-coding the long-run mean. It converges to
...
function expected_mu_path(E::Eff)
    ...
    for t in 1:E.NT
        ...
        dist = E.hP' * dist          # advance the regime distribution one year
    end
    return mean_mu
end
